...
export default function LoginPage() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      if (mode === "up") {
        const { data, error: err } = await db.auth.signUp({ email: email.trim(), password, options: { data: { display_name: name.trim() || undefined }, emailRedirectTo: `${window.location.origin}/today` } });
        if (err) throw err;
